...
char* duplicate_sentences(const char* text, int n) {
    ...
    char* result = (char*)malloc(text_len * n * 2 + n + 1);
    if (result == NULL) {
    ...
    char* text_copy = strdup(text); // Исправлено: используем strdup
    if (text_copy == NULL) {
        fprintf(stderr, "Ошибка выделения памяти\n");
        free(result);
        exit(1);
...
int main() {
    ...
    if (command == 5) {
        ...
        scanf("%d\n", &n);
        int num_sentences;
        char** text = read_text(&num_sentences, 1);
        ...
        getchar();
        int num_sentences;
        char** text = read_text(&num_sentences, 0);
